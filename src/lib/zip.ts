...
export async function packTo(file: string, codeignore: any, targetPath: string, prefix = '', zlibOptions = {}) {
  ...
  logger.debug(`append ${stats.isFile() ? 'file' : 'folder'}: ${file}, absolute path is ${path.resolve(file)}`);
...
async function zipFolder(zipArchiver, folder, folders, codeignore, codeUri, prefix = '') {
  ...
  return (await Promise.all(dirItems.map(async (f) => {
    ...
    logger.debug(`before zip: lstat fPath: ${fPath}, absolute fPath is ${path.resolve(fPath)}`);
    ...
    try {
      ...
      logger.debug(`before zip: could not found fPath ${fPath}, absolute fPath is ${path.resolve(fPath)}, exception is ${error}, skiping`);
      return 0;
    }
    if (codeignore && codeignore(fPath)) {
      logger.debug(`file ${fPath} is ignored.`);
      return 0;
    }
