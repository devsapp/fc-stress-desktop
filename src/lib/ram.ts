...
import { CONTEXT, RETRYOPTIONS } from './static';
import { ICredentials, IProperties, IPolicy, IRoleDocument } from '../common/entity';
...
export default class AlicloudRam {
  ...
  async checkPolicyNotExistOrEnsureAvailable(
    ...
    this.logger.info(`Checking policy ${policyName}`);
  ...
  async updatePolicy(policyName: string, statement: any) {
    this.logger.info(`Updating policy: ${policyName}`);
    ...
    await retry(async (rty: (arg0: any) => void, times: any) => {
      try {
        ...
        this.logger.info(`Retry updating policy for ${times} times`);
        rty(ex);
      }
    }, RETRYOPTIONS);
